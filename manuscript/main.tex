...
As a third example, illustrating that contribution caps are useful well beyond the additive, Gaussian setting of the previous two examples, consider a count-regression model that forecasts aggregate web traffic from a panel of individual, heavy-tailed content-level pageview series. Article-level popularity on the web is well documented to be Zipf-like and heavy-tailed: a small number of pages can go viral and receive orders of magnitude more traffic than usual on a given day. A Poisson log-link model that forecasts tomorrow's site-wide traffic from today's per-article pageviews is exactly the setting \pkg{capnet} targets: without a contribution cap, a single freak spike in one article's traffic can dominate the forecast, even though the associated coefficient need not be unusually large under a standard fit. Because the response enters through a log link in this setting, contribution caps bound a \emph{multiplicative} rather than additive influence on the forecast, as discussed in Section~\ref{sec:framework}. Further details are discussed in Section~\ref{sec:examples-web}.
...
\begin{Code}
    R> set.seed(123)
    R> data_pois <- simulate_capnet_data(n = 100, p = 10, m = 20, family = "poisson")
...
\begin{Code}
    R> data_bin <- simulate_capnet_data(n = 100, p = 10, m = 20, family = "binomial")
    R> fit_bin <- capnet(data_bin$X, data_bin$y, data_bin$L, z = data_bin$Z,
    +    family = "binomial", alpha = alpha, lambda = lambda, gamma = gamma)

    R> data_gamma <- simulate_capnet_data(n = 100, p = 10, m = 20, family = "gamma")
    R> fit_gamma <- capnet(data_gamma$X, data_gamma$y, data_gamma$L, z = data_gamma$Z,
    +    family = "gamma", alpha = alpha, lambda = lambda, gamma = gamma)
\end{Code}

A fully worked empirical application of the Poisson case, on real (non-simulated) data, is presented in Section~\ref{sec:examples-web}.
...
A second application of \pkg{capnet}, outside quantitative finance, arises in forecasting aggregate web traffic from a panel of individual, heavy-tailed content-level pageview series. Article-level popularity on the web is well documented to be Zipf-like and heavy-tailed: a small number of pages can go viral and receive orders of magnitude more traffic than usual on a given day \citep{crovella1997}. A count-regression model that forecasts tomorrow's site-wide traffic from today's per-article pageviews is exactly the setting \pkg{capnet} targets: without a contribution cap, a single freak spike in one article's traffic can dominate the forecast, even though the associated coefficient need not be unusually large under a standard fit.

To illustrate this, we use daily English Wikipedia pageviews (January 1, 2022 through December 31, 2024) for 30 widely-read articles spanning major technology companies, artificial-intelligence and cryptocurrency topics, and prominent public figures, obtained from the public Wikimedia Pageviews REST API. The response is next-day total pageviews summed across all 30 articles; the covariates are each article's own pageviews (in thousands of views) on the current day. Table~\ref{tab:wiki_variable} reports summary statistics for a representative subset of the covariate panel.
...
As a robustness check on this result, we repeat the backtest restricted to calendar year 2024 only, which excludes the March 2023 event entirely. Over this shorter window the uncapped model is no longer catastrophic (OOS pseudo-$R^2 \approx 0.540$), but the walk-forward \pkg{capnet} model still achieves a modest, consistent improvement (OOS pseudo-$R^2 \approx 0.569$ at $\gamma = 10{,}000$), while post-fit capping remains the weakest approach throughout (OOS pseudo-$R^2 \approx -1.11$). This indicates that the benefit of \pkg{capnet} is not solely an artifact of avoiding one extreme event: even absent a catastrophic blowup, incorporating the contribution cap directly into estimation yields a systematically more reliable fit than either the uncapped or post-fit alternative.
