...
Substituting $Z_i = X_i - \eta_i$, the response can equivalently be written in terms of the observed covariates as
\[
y_i = X_i\beta + u_i,
\]
where
...
\]
Thus, when the analyst estimates the model using the observed design matrix $X$, the fitted model is based on covariates that are noisy proxies for the latent signal $Z$. Because the measurement error for unstable features occasionally has much larger variance, the observed covariates inherit heavy-tailed behavior. These tail realizations can generate large realized contributions $X_{ij}\beta_j$, even when the underlying latent features are more stable.
