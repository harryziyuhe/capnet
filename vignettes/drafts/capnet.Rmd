...
vignette: >
  %\VignetteIndexEntry{Contribution Constrained Regularization}
...
$$\arg\min_{\beta}||Y-X\beta||_2^2+\lambda\Big(\alpha||\beta||_1+(1-\alpha)||\beta||_2^2\Big)+\gamma\sum_{i=1}^k\Big|\Big|\max\big(0,|\beta_iZ_i|-L\big)\Big|\Big|_p^p$$
...
$$\min_{\beta_0,\beta}\frac{1}{2N}\sum_{i=1}^N\Big(y_i-(\beta_0+x_i^T\beta)\Big)^2+\lambda\bigg[\frac{(1-\alpha)}{2}||\beta||_2^2+\alpha||\beta||_1\bigg]+\gamma\sum_{i=1}^k\max\big(0,|\beta_iZ_i|-L\big)^2$$
...
#### Convergence Failure
Because `capnet` relies on the `lbfgs` package for optimization, convergence failure can be caused by multiple factors and may be hard for users to detect and debug. Importantly, the function does not stop or raise an error when convergence fails, so users are advised to manually inspect `fit$convergence` and `fit$message` after fitting; a `convergence` value other than `0` indicates the optimizer did not reach its usual stopping criterion. Because `lbfgs` does not store a descriptive message for every failure code, the message output may not always explain why convergence failed. Generally, when users encounter a $-998$ error code, it is usually that the line search algorithm failed because the gradient values are too unstable -- reducing `gamma`, `lambda`, or rescaling ill-conditioned features can help.
